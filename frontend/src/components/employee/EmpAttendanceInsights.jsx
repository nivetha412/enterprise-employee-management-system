...
const NOTIFICATIONS = [
  { icon: "⚠️", text: "Missing check-out yesterday",       time: "1h ago", unread: true  },
  { icon: "📋", text: "Attendance correction approved",     time: "3h ago", unread: true  },
  { icon: "🎉", text: "Independence Day holiday on Jul 4",  time: "1d ago", unread: false },
  { icon: "📊", text: "June attendance report available",   time: "2d ago", unread: false },
];
...
export default function EmpAttendanceInsights() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      {/* Insights */}
      <div style={{ background: "#fff", borderRadius: "16px", boxShadow: "0 4px 24px rgba(30,64,175,0.08)", border: "1px solid #e8edf5", overflow: "hidden" }}>
        ...
        <div style={{ padding: "14px 22px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
          {INSIGHTS.map(({ icon: Icon, label, value, sub, color, bg, border }) => (
            <div key={label} style={{ background: bg, borderRadius: "12px", padding: "14px", border: `1px solid ${border}60`, transition: "transform 0.2s, box-shadow 0.2s", cursor: "default" }}
      ...
      <div style={{ background: "#fff", borderRadius: "16px", boxShadow: "0 4px 24px rgba(30,64,175,0.08)", border: "1px solid #e8edf5", overflow: "hidden" }}>
        ...
        <div style={{ padding: "14px 22px", display: "flex", flexDirection: "column", gap: "8px" }}>
          {[
            ...
            <button key={label} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "11px 14px", borderRadius: "11px", background: bg, border: `1px solid ${border}`, cursor: "pointer", transition: "all 0.15s", width: "100%", textAlign: "left" }}
      ...
      <div style={{ background: "#fff", borderRadius: "16px", boxShadow: "0 4px 24px rgba(30,64,175,0.08)", border: "1px solid #e8edf5", overflow: "hidden" }}>
        ...
        <div style={{ padding: "14px 22px", display: "flex", flexDirection: "column", gap: "8px" }}>
          {HOLIDAYS.map((h, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 12px", borderRadius: "11px", background: "#f8fafc", border: "1px solid #f1f5f9", borderLeft: `3px solid ${h.color}`, transition: "all 0.15s", cursor: "pointer" }}
              ...
              <div style={{ width: "40px", height: "40px", borderRadius: "9px", background: h.color + "15", border: `1px solid ${h.border}50`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: "9.5px", fontWeight: 700, color: h.color, background: h.bg, padding: "2px 8px", borderRadius: "10px", border: `1px solid ${h.border}` }}>Holiday</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: "16px", boxShadow: "0 4px 24px rgba(30,64,175,0.08)", border: "1px solid #e8edf5", overflow: "hidden" }}>
        ...
        <div style={{ padding: "8px 0" }}>
          {NOTIFICATIONS.map((n, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "10px", padding: "10px 22px", background: n.unread ? "#f8fbff" : "transparent", borderBottom: i < NOTIFICATIONS.length - 1 ? "1px solid #f8fafc" : "none", cursor: "pointer", transition: "background 0.15s" }}
